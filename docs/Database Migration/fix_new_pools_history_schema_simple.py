...
Simple version using direct psycopg2 connection.
"""
...
async def fix_new_pools_history_schema():
    ...
    db_url = os.getenv('DATABASE_URL', 'postgresql://gecko_collector:12345678!@localhost:5432/gecko_terminal_collector')
    ...
    try:
        ...
        current_columns = {row['column_name'] for row in columns_info}
        ...
        for column_name in sorted(missing_columns):
            try:
                ...
                await conn.execute(required_columns[column_name])
        ...
        final_columns_info = await conn.fetch("""
            SELECT column_name, data_type, is_nullable, column_default
            FROM information_schema.columns 
            WHERE table_name = 'new_pools_history' 
            ORDER BY ordinal_position;
        ...
        final_columns = {row['column_name'] for row in final_columns_info}
