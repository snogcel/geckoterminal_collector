#!/usr/bin/env python3
"""
Test script for enhanced database manager without migrations.
"""
...
async def test_enhanced_database_no_migration():
    """Test the enhanced database manager functionality without migrations."""
    
    # Create test database configuration
    db_config = DatabaseConfig(
        url=f"sqlite:///test_no_migration_{int(datetime.utcnow().timestamp())}.db",
        pool_size=5,
        max_overflow=10,
        timeout=30
    )
    
    # Initialize enhanced database manager (this will create tables)
    ...
    try:
        ...
        if active_alerts:
            ...
            logger.info("No active alerts to resolve")
...
if __name__ == "__main__":
    asyncio.run(test_enhanced_database_no_migration())