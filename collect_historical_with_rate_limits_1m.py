...
async def collect_historical_with_delays(
    ...
    try:
        ...
        if pool_ids is None:
            ...
            # Historical collector adds network prefix internally, so we need to strip it
            # Watchlist stores: solana_abc123
            # Collector expects: abc123 (it will add solana_ back when storing)
            pool_ids = [entry.pool_id.replace('solana_', '') for entry in entries]

            print(pool_ids)

            print(f"Collecting for all {len(pool_ids)} watchlist pools")
            print(f"Sample pool IDs (without prefix): {pool_ids[:3] if len(pool_ids) >= 3 else pool_ids}")
        else:
            print(f"Collecting for {len(pool_ids)} specified pools")
        
        # Use default timeframes if not specified
        if timeframes is None:
            timeframes = ['1m']  # Start with less frequent timeframes
...
async def collect_missing_timeframes():
    ...
    try:
        ...
        for entry in entries:
            # Strip prefix for collection (collector adds it back)
