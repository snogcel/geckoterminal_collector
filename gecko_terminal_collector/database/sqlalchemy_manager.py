...
class SQLAlchemyDatabaseManager(DatabaseManager):
    ...
    def optimized_session(self, read_only=False):
        ...
        try:
            ...
            if "database is locked" in str(e).lower():
                logger.warning(f"Database lock detected in optimized session")
                session.rollback()
                raise
            else:
                session.rollback()
    ...
    async def store_pools_optimized(self, pools: List[Pool]) -> int:
        """
        Optimized pool storage with lock avoidance and batch processing.
        """
        if not pools:
            return 0
        
        return await self._retry_with_backoff(self._store_pools_batch_optimized, pools)
    
    async def _store_pools_batch_optimized(self, pools: List[Pool]) -> int:
    ...
    def _pool_to_dict(self, pool) -> Dict[str, Any]:
        ...
        return {
            ...
            'discovery_source': getattr(pool, 'discovery_source', None),
            'collection_priority': getattr(pool, 'collection_priority', None),
            'auto_discovered_at': getattr(pool, 'auto_discovered_at', None),
            'last_activity_check': getattr(pool, 'last_activity_check', None)
        }
    ...
    async def store_trade_data_optimized(self, data: List[TradeRecord]) -> int:
        ...
        Key optimizations:
        1. Pre-filter existing records with minimal lock time
        ...
        return await self._retry_with_backoff(self._store_trades_batch_optimized, data)
            ...
            try:
                ...
                try:
                    ...
                    logger.info(
                        ...
                        f"{'🔔 TRIGGERING NOTIFICATION (2nd+ occurrence)' if history_count >= 2 else '⏳ waiting for 2nd occurrence'}"
                    )
                    
                    # Return True only if this is the 2nd or later occurrence
    ...
    async def get_database_health_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if "database is locked" in str(e).lower():
                metrics['lock_contention_detected'] = True
                metrics['connection_status'] = 'lock_contention'
            else:
